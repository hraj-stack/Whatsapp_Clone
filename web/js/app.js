...
async function renderContactForm(root, contactId) {
        ...
        <div class="avatar-upload-wrap">
          <div class="avatar-upload" id="avatar-upload" role="button" tabindex="0"
            ...
            <div id="avatar-preview-container">
              ${avatarHTML({ name: contact?.name || '', avatar: contact?.avatar }, 'avatar-xl')}
            </div>
  ...
  const updatePreview = () => {
    ...
    previewContainer.innerHTML = avatarHTML({ name, avatar: selectedAvatar }, 'avatar-xl');
  };
...
// Predefined dark palette for default avatars (flat, good contrast with white text)
...
/**
 * Deterministically select a background color using character code summation.
 */
function getAvatarColor(key) {
  const norm = (key || '').trim();
  if (!norm) return AVATAR_PALETTE[0];
  let sum = 0;
...
/**
 * Extract the first letter or number of the contact name.
 * Letters converted to uppercase. Empty/missing name returns '?'.
 */
 ...
 * - sizeClass: avatar-sm (40px), avatar-md (48px), avatar-lg (56px), avatar-xl (64px)
 * - isSaved: boolean (true = saved with name, false = unsaved phone number)
 */
function avatarHTML(contact, sizeClass = 'avatar-md', isSaved = true) {
  const name      = contact.name || '';
  const phone     = contact.phone || '';
  const avatarUrl = contact.avatar || null;

  // 1. If profile picture exists, show it
  if (avatarUrl) {
    return `<img src="${avatarUrl}" alt="${escapeHTML(name || phone)}" class="avatar ${sizeClass}" />`;
  }

  // 2. Determine background color based on name (saved) or phone (unsaved)
  const hashKey = isSaved ? name : phone;
  const color   = getAvatarColor(hashKey);

  // 3. Render placeholder depending on saved status
  if (isSaved) {
    // Saved contact: show first letter of the name
    const char = getAvatarChar(name);
    return `<div class="avatar ${sizeClass} avatar-placeholder" style="background-color: ${color}" aria-label="Avatar for saved contact ${escapeHTML(name)}">${char}</div>`;
  } else {
    // Unsaved contact: show generic person silhouette icon (flat design, centered, no border)
    const silhouetteIcon = `<svg viewBox="0 0 24 24" fill="#FFFFFF" style="width: 55%; height: 55%; pointer-events: none;"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>`;
    return `<div class="avatar ${sizeClass} avatar-placeholder" style="background-color: ${color}" aria-label="Avatar for unsaved contact ${escapeHTML(phone)}">${silhouetteIcon}</div>`;
  }
}
